...
//        LOG.info("job : adid={} udid={} | remain num : {}", adid, udid, remain);
//
//		int isPay = 0;
...
//			if (media.getType() == 1) {
//				MediaApp app = (MediaApp) media;
//				UserScore.sendScore(app, ad, item.appuserid, item.mac, item.udid, item.openudid, item.score, ad.getProcess_name(), active_num, isPay);
//				LOG.info("job : adid={} udid={} | user send score " + item.uid + " " + item.appid + " " + item.appuserid + " " + item.mac + " " +
//                        item.udid + " " +item.openudid + " " +item.score, adid, udid);
//			} else {
