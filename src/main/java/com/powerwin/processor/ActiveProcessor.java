...
//        }
//        type = ad.getDataType();
//        if (ad.getIsHsFlag() == 3 && isEnable == 0) {
//            // 快速精准控量 补偿逻辑不再精准控量
...
//                LOG.debug("active wait idfa is not exist adid={} udid={} idfa={}", adid, udid, tidfa);
//                return null;
//            } else {
...
//        LOG.info("active : adid={} udid={} | remain num : {}", adid, udid, remain);
...
//            // 广告由进程激活后广告主回调激活
//            if ((ad.getProcess_name() != null && !ad.getProcess_name().isEmpty()) && item.process == Define.FROM_PROCESS
...
//                            item.appid, item.uid, item.adid, item.cid);
//
...
//
//        if (media.getIsEnable() == 0 && isEnable == 0) { // 启用
//            // 获取广告防作弊设置 ad by chenjun
...
//             * 激活DID防作弊判断
//             */
//            if (invalid == 0 && ADS_OPTIONS.get(Cheat.KEY_SESSION) == 1) { // add
//                                                                           // by
//                                                                           // chenjun
...
//             * 防作弊：SSID 激活判断
//             */
//            if (invalid == 0 && ADS_OPTIONS.get(Cheat.KEY_SSID) == 1) { // add
//                                                                        // by
//                                                                        // chenjun
...
//             * 防作弊：OPENUDID 激活判断
//             */
//            if (invalid == 0 && ADS_OPTIONS.get(Cheat.KEY_OPENUDID) == 1) { // add
//                                                                            // by
//                                                                            // chenjun
...
//             * 防作弊软件激活的判断
//             */
//            if (invalid == 0 && ADS_OPTIONS.get(Cheat.KEY_IG) == 1) { // add by
//                                                                      // chenjun
...
//             * 磁盘空间变化
//             */
//            if (invalid == 0 && disk > 0 && ADS_OPTIONS.get(Cheat.KEY_DISK) == 1) { // add
//                                                                                    // by
//                                                                                    // chenjun
...
//
//        // 进程激活
//        if ((ad.getProcess_name() != null && !ad.getProcess_name().isEmpty()) && source == Define.FROM_PROCESS) {
//            // 上报数据但不计入广告主数据
//            if (ad.getIsHsReport() == 2 || ad.getIsHsReport() == 0) {
//                count = 0;
//                income = 0;
...
//                    LOG.info(
//                            "active : adid={} udid={} | insert process info to redis[DATA_DEVICE_ADID], key={},value={}",
//                            adid, udid, field, v);
//                } else {
...
//                        jedisProcess.hset(keyProcess, field, v);
//                        LOG.info(
//                                "active : adid={} udid={} | update process info to redis[DATA_DEVICE_ADID], key={},value={}",
//                                adid, udid, field, v);
//                    }
//                }
//            } catch (Exception e) {
//                LOG.warn("active :  ={} udid={} | update process info to redis[DATA_DEVICE_ADID] error : {}", adid,
//                        udid, e.getMessage());
//            } finally {
...
//                    obj.put("type", "sdk");
//                    obj.put("url", httpUrl);
//                    LOG.debug("active : adid={} udid={} | report idfa to cp : {}", ad.getAdid(), item.udid, httpUrl);
...
//            }
//        } catch (Exception e) {
//            LOG.error("report idfa[{}] to cp[{}] error : {}", item.udid, ad.getAdid(), e.getMessage());
//        }
    }
...
//                String sIncome = "0";
//                if (jsonObj.get("income").toString().isEmpty() || jsonObj.get("income").toString().length() == 0) {
//                    LOG.debug("active income error :  adid : " + ad.getAdid());
//                } else {
//                    sIncome = jsonObj.get("income").toString();
//                }
//                income = Float.parseFloat(sIncome);
...
//                }
//
//                cost = this.getPrice(item.appid, ad.getAdid());
//                if (cost == 0) {
//                    cost = Float.parseFloat(job.get("const").toString());
//                    if (cost == 0) {
//                        cost = ad.getPriceCallbackCost();
...
//            income = ad.getPriceCallbackIncome();
//            cost = ad.getPriceCallbackCost();
//            LOG.warn(String.format("active adid=%d,exception=%s", ad.getAdid(), e.getMessage()));
//        }
